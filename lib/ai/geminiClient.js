...
/**
 * Multi-turn function-calling loop — the AI Analyst brain (spec §4).
 * `tools` are Gemini functionDeclarations [{name, description, parameters}].
 * `executeTool(name, args) => string|object` runs the tool and its result is
 * fed back until Gemini produces a final text answer.
 * Returns { answer, toolsUsed }.
 */
async function geminiToolLoop(messages, tools, executeTool, opts = {}) {
  const { systemInstruction, contents } = toGemini(messages);
  const toolDecls = tools && tools.length ? [{ functionDeclarations: tools }] : undefined;
  const toolsUsed = [];
  ...
  for (let turn = 0; turn < maxTurns; turn++) {
    const body = {
      contents,
      generationConfig: { temperature: opts.temperature ?? 0.3, maxOutputTokens: opts.maxTokens ?? 8192 },
    };
    if (systemInstruction) body.systemInstruction = systemInstruction;
    if (toolDecls) body.tools = toolDecls;

    const data = await generateContent(body, opts);
    ...
    // Record the model's function-call turn, then answer each call.
  ...
  throw new Error(`Gemini tool loop exceeded ${maxTurns} turns`);
}

function geminiConfigured() { return keys().length > 0; }

module.exports = { geminiText, geminiJSON, geminiToolLoop, generateContent, toGemini, keys, geminiConfigured };
