...
// Entry point of the vendored 134-tool server (@mikusnuz/meta-ads-mcp). Overridable
// via META_MCP_SCRIPT for a different server.
function metaMcpEntry() {
  return process.env.META_MCP_SCRIPT
    || path.join(__dirname, '..', '..', 'node_modules', '@mikusnuz', 'meta-ads-mcp', 'dist', 'index.js');
}

async function createMetaMCPClient(metaToken, adAccountId) {
  // Inject THIS org's token into the MCP server's environment — workspace isolation:
  // every org gets its own server instance scoped to its token.
  const env = {
    ...process.env,
    META_APP_ID:           process.env.META_APP_ID,
    META_APP_SECRET:       process.env.META_APP_SECRET,
    META_ADS_ACCESS_TOKEN: metaToken,                       // mikusnuz reads this
    META_ACCESS_TOKEN:     metaToken,                       // harmless for other servers
    META_AD_ACCOUNT_ID:    (adAccountId || '').replace(/^act_/, ''),
  };

  const transport = new StdioClientTransport({
    command: process.env.META_MCP_COMMAND || 'node',
    args:    [metaMcpEntry()],
    env,
  });
