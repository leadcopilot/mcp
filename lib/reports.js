...
async function monthlyReport(orgId) {
  const [k, data] = await Promise.all([getOrgKnowledge(orgId), dashboardSummary(orgId)]);
  const sys = 'You are the LeadPilot Ad Manager. Write a concise monthly performance report for the founder, grounded ONLY in the provided data — never invent numbers. Sections: Executive Summary, Lead Performance, Call Quality, Recommendations for next month.';
  ...
  return { org: k?.business_name, generated_at: new Date().toISOString(), data, report };
}
